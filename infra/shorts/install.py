...
def install(account,project,region,bucket):
    ...
    with tempfile.TemporaryDirectory(prefix='anime-shorts-install-') as tmp:
        ...
        deploy=['run','deploy',service,'--image',immutable,'--region',region,'--project',project,'--service-account',sa,'--command=gunicorn','--args=--bind,:8080,--workers,2,--threads,4,--timeout,1800,shorts.service.app:app','--env-vars-file',str(envfile),'--cpu=1','--memory=1Gi','--timeout=1800s','--concurrency=8','--max-instances=2','--allow-unauthenticated','--tag=candidate-'+sha[:12],'--no-traffic','--labels=managed-by=anime-shorts-v2','--quiet']
