...
- El proyecto selecciona y guarda una **fórmula viral de referencia** según concepto/género: apocalipsis+base/sistema, divorcio+retorno de estatus, compromiso+asimetría de información, bestia+evolución, cultivación+face-slap, oficio humilde+habilidad imposible o fórmula general
- El motor no premia la originalidad: conserva la fantasía de audiencia, progresión, antagonistas, inversiones de estatus y payoffs que funcionaron en las referencias, sin copiar texto literal
- Cada bloque largo recibe un hito obligatorio de esa fórmula y sólo se acepta si incluye dos pruebas literales del propio guion: `viralEvidence` (donde ocurre el hito) y `viralConsequence` (una reacción o consecuencia posterior). Ambas deben existir dentro del texto y en ese orden; los proyectos anteriores se migran automáticamente al motor viral final al regenerar
