...
def acquire(job,project,slot,now):
    ...
    require(not slot or slot.get('jobId') in (None,job['id']),'CAPACITY_BUSY','Otro trabajo usa este motor. El pendiente conserva su reserva.',429)
    if job['operation'] in PAID:
