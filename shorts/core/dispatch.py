...
def acquire(job,project,slot,now):
    ...
    if job['operation'] in GENERATION_OPERATIONS:
        require(project.get('lease',{}).get('session')==job['session'] and project['lease']['expires']>now,'LEASE','La sesión se pausó antes del arranque. Pulsa Continuar en este trabajo.',409)
