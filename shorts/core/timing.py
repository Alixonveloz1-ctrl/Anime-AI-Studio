...
def voice_layout(shot, voices):
    cursor=integer(shot.get('leadFrames',0),'acción inicial',0,7200)*2000
    rows=[]
    for utterance,audio in voices:
        cursor+=integer(utterance.get('pauseBeforeFrames',0),'pausa antes',0,7200)*2000
        length=integer(audio['samples'],'voz medida',1,14400000)
        rows.append({'utterance':utterance,'audio':audio,'offsetSample':cursor})
        cursor+=length+integer(utterance.get('pauseAfterFrames',0),'pausa después',0,7200)*2000
    cursor+=integer(shot.get('tailFrames',0),'acción final',0,7200)*2000
