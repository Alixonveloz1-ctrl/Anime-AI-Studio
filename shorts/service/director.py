...
def validate_development(d):
    ...
    def text(row,key):
        label='la descripción visual de referencia' if key=='referencePrompt' else key
        name=row.get('name') or row.get('id') or 'una ficha del guion'
        require(isinstance(row.get(key),str) and bool(row[key].strip()),'DEVELOPMENT_TEXT',f'La respuesta de Google está incompleta: falta {label} en «{name}».')
