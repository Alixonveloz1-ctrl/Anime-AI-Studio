...
async function assetCard(parent,a){
 const spoken=development()?.utterances.find(u=>u.id===a.entityId);const c=card(spoken?spoken.speakerId+' · '+spoken.spanish:a.entityId,`<p class="badge">${esc(a.kind)} · ${esc(a.approvalState)}${selectedAsset(a.entityId,a.kind)?.id===a.id?' · versión en uso':''}</p>`);
 if(a.requestId&&a.samples){const requested=development()?.soundRequests.find(x=>x.id===a.requestId);const duration=document.createElement('p');duration.textContent=`Duración solicitada: ${requested?.seconds??'—'} s · recibida: ${(a.samples/48000).toFixed(3)} s. No se estira automáticamente.`;c.append(duration);}
...
async function shots(){
 ...
 for(const a of p.assets.filter(a=>['characters','locations','props'].some(k=>d.bible[k].some(x=>x.id===a.entityId))))await assetCard(screen,a);
 ...
 for(const a of p.assets.filter(a=>a.entityId===s.id||a.variantOf===s.id||d.utterances.some(u=>u.shotId===s.id&&u.id===a.entityId)))await assetCard(c,a);screen.append(c);}
}
...
async function sounds(){
 ...
 for(const m of d.musicRequests){const c=card('Música',`<p>${esc(m.prompt)}</p><p>${m.seconds} segundos · pieza reutilizable</p>`);buttons(c,[['Generar pieza instrumental',()=>paid('music',route('/assets:generate'),{operation:'music',entityId:m.id})]]);for(const a of p.assets.filter(a=>a.entityId===m.id))await assetCard(c,a);screen.append(c);}
 for(const r of d.soundRequests){
  ...
  for(const cue of p.cues.filter(c=>c.requestId===r.id)){buttons(c,[['Ajuste manual',()=>openEditor(cue)],['Localizar con IA',()=>paid('analyze',route(`/cues/${cue.id}:analyze`)),true],['Corregir sonido con IA',async()=>{const reason=prompt('Describe el problema de este efecto (por ejemplo: suena después del contacto).');if(reason)await paid('analyze',route(`/cues/${cue.id}:correct`),{reason});},true],['Usar propuesta IA',async()=>{await mutate(route(`/cues/${cue.id}/proposal:apply`),{});say('Propuesta aplicada como candidata.');},true]]);
