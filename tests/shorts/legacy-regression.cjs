...
// generation and montage bodies must remain identical. Storage adds only the
// separately tested recovery actions and immutable backup-before-save hook. U006 also
...
for(const name of fs.readdirSync('api').filter(x=>x.endsWith('.js')&&x!=='shorts.js')){
 ...
 const normalized=source.replace(/      \/\/ Owner-only metadata recovery; existing generation endpoints are unchanged\.\n[\s\S]*?(?=      if \(action === 'projectSave'\))/, '').replace("        await require('./_lib/project-recovery').preserveManifest(token, id, existing);\n",'').replace('if (await begin(req, res','if (begin(req, res').replace("  if (await require('./_lib/access').handleSession(req, res)) return;\n",'').replace(/^        \/\/ Load diagnostics contain counts only, never story text or media URLs\.\n        console\.info\('\[animes:project-load\]'[^\n]*\n/m,'');
...
console.log('A003/A006/A008/A085 + U005/U006: access and tested loading fixes only; Animes design, generation logic, provider helpers, generation APIs, installers and worker unchanged; recovery/backup changes are explicitly bounded and tested. Real project export remains pending.');
